...
import org.GitHub.ResponsePOJO.errors;
...
public void negativeAddDataFunctionality() {
   ...
   // NegativeCreateMsg negative = response.as(NegativeCreateMsg.class); 
    //System.out.println(negative.getError().get(0).getMessage());
    /*ArrayList<ErrorMessage> list = negative.;
    for(int i=0;i<list.size();i++) {
    String Message = list.get(i).message.toString();
    System.out.println(Message);}
    //report.logTestInfo("Error Message" + ErrorMessage);
   // Assert.assertEquals(Message, "name already exists on this account", "Error Message not displayed");
    System.out.println("The message is " + list.get(0).getMessage());*/
    NegativeCreateMsg neg =response.getBody().as(NegativeCreateMsg.class);
    System.out.println(neg.getErrorsmessage().get(0).getErrmessage());
...
public void negativeDeleteData() {
   ...
   //String Actual =data.body().asPrettyString();
